...
class QLIndependentBelief2(QLRMAlgo):
  ...
  def initialize_q_table(self):
    ...
    self.q2 = { (rm_state, pos, pos_op, a) : 0 if rm_state == 2 or pos == depot else (np.random.random() - 0.5) 
               for rm_state in range(3) 
               for pos in range(board_lenght) 
               for pos_op in range(board_lenght)
               for a in range(action_lenght) }
    
    self.q3 = { (rm_state, pos, pos_op, dug_pos, a) : 0 if rm_state == 2 or pos == depot else (np.random.random() - 0.5) 
               for rm_state in range(3) 
               for pos in range(board_lenght) 
               for pos_op in range(board_lenght)
  ...
  def get_q_value(self, state, rm_belief, action):
    ...
    for u in range(3):
      ...
      for dug_pos in range(dug_lenght):
        q_sum += self.q3[(u, state[0], state[3], dug_pos, action)] * rm_belief[0][u] * state[2][dug_pos] / dug_lenght
    return q_sum
  ...
  def update_q_values(self, state, rm_belief, action, reward, next_state, next_rm_belief, done):
    ...
    for u in range(3):
      ...
      for dug_pos in range(dug_lenght):
        self.q3[(u, state[0], state[3], dug_pos, action)] += self.lr * delta * rm_belief[0][u] * state[2][dug_pos] / dug_lenght
